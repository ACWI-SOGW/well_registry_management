...
def _get_monitoring_location(row_index, row, user, warning_messages):
    ...
    monitoring_location = MonitoringLocation(
        ...
        site_no=row[1],
        site_name=row[2],
        dec_lat_va=_validate_decimal('dec_lat_va', row[3], row_index, warning_messages),
        dec_long_va=_validate_decimal('dec_long_va', row[4], row_index, warning_messages),
        horizontal_datum=_get_lookup(HorizontalDatumLookup, 'hdatum_cd', row[5]),
        horz_method=row[6],
        horz_acy=row[7],
        alt_va=_validate_decimal('alt_va', row[8], row_index, warning_messages),
        ...
        well_depth= _validate_decimal('well_depth', row[19], row_index, warning_messages),
    ...
    def post(self, request):
        ...
        if 'file' in request.FILES:
            ...
            for row in csv.reader(data_stream):
                row_index += 1
                try:
                    monitoring_locations.append(_get_monitoring_location(row_index,row, request.user, warning_messages))
            ...
            if len(error_messages) == 0:
                ...
                if len(warning_messages) == 0:
                    ...
                    warning_messages.insert(0, (0, {'__overall__': 'Data was loaded with the following warnings'}))
