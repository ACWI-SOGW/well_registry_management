...
class MonitoringLocationAdmin(ModelAdmin):
    ...
    def get_urls(self):
        ...
        custom_urls = [
            ...
            path('fetch_from_nwis/', self.admin_site.admin_view(
                FetchFromNwisView.as_view()), name='fetch_from_nwis'),
    ...
    def get_usergroups(self, request):
        user_groups = ""
    ...
    def get_where_clause(self, request, user_groups):
        where_clause = ""
        sep = ""
        query_params = request.body.decode().split("&")
        for query_param in query_params:
            monitoringlocation_ids = query_param.split("=")
            if monitoringlocation_ids[0] == "_selected_action":
                where_clause += sep + monitoringlocation_ids[1]
                sep = ","
        if len(where_clause) > 0:
            if user_groups == "":
                ...
                where_clause = user_groups + \
                    " and registry_monitoringlocation.id IN (" + \
                    where_clause + ") \n"
        return where_clause

    def get_monitoringlocation_queryset(self, request):
        user_groups = self.get_usergroups(request)
        where_clause = self.get_where_clause(request, user_groups)
    ...
    def download_monitoring_locations(self, request, queryset):
        ...
        query_monitoring_location = self.get_monitoringlocation_queryset(
            request)
